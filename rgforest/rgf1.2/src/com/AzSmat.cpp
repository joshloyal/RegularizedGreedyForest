...
void AzSmat::_from_buffer(double* buf, int inp_row_num, int inp_col_num)
{
  const char *eyec = "AzSmat::_from_buffer";
  if (inp_col_num < 0 || inp_row_num < 0) {
    throw new AzException(eyec, "#column and #row must be non-negative");
  }
  if (column != NULL || row_num > 0) {
    throw new AzException(eyec, "_release() must be called before this");
  }

  // the input buffers are in (n_samples, n_feature) format
  // The AzSmat needs to be in (n_feature, n_sample) format
  col_num = inp_row_num;
  row_num = inp_col_num;
  a.alloc(&column, col_num, eyec, "column");
  int rx, cx; // data is kept in order (n_features, n_samples) so a column is a row of data
  double row[inp_col_num];
  for(rx = 0; rx < inp_row_num; ++rx) {
    for (cx = 0; cx < inp_col_num; ++cx) {
      row[cx] = buf[rx * inp_col_num + cx];  // assumes buf is in row-major order (c order)
    }
    column[rx] = new AzSvect(row, inp_col_num);
  }
  dummy_zero.reform(row_num);
}
...
void AzSvect::_from_buffer(double* data, int inp_row_num) {
  // dense intitialization from buffer
  const char *eyec = "AzSvect::_from_buffer";
  if (inp_row_num < 0) {
    throw new AzException(eyec, "#row must be non-negative");
  }
  if (elm != NULL || elm_num > 0) {
