...
public:
  ...
  AzSvect(double* data, int inp_row_num)
            : row_num(0), elm(NULL), elm_num(0) {
    _release();
    _from_buffer(data, inp_row_num);
  }
  void _from_buffer(double* data, int inp_row_num);
...
public:
  ...
  AzSmat(double* data, int inp_row_num, int inp_col_num)
    : col_num(0), row_num(0), column(NULL) {
    _release();
    _from_buffer(data, inp_row_num, inp_col_num);
  }
  void _from_buffer(double* data, int inp_row_num, int inp_col_num);
