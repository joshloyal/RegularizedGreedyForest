...
public:
  ...
  AzSvDataS(double* features, double* target, int n_rows, int n_cols)
            : max_data_num(-1), const_to_add(0) {
      _from_buffer(features, target, n_rows, n_cols);
  }
  void _from_buffer(double* features, double* target, int n_rows, int n_cols);
