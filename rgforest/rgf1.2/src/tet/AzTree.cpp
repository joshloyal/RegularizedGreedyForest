...
void AzTree::_from_nodes(Node* inp_nodes) {
  const char *eyec = "AzTree::_from_node";
  a_nodes.alloc(&nodes, nodes_used, eyec, "nodes");
  int nx;
  for (nx = 0; nx < nodes_used; ++nx) {
      nodes[nx].parent_nx = inp_nodes[nx].parent;
      nodes[nx].le_nx = inp_nodes[nx].left_child;
      nodes[nx].gt_nx = inp_nodes[nx].right_child;
      nodes[nx].fx = inp_nodes[nx].feature;
      nodes[nx].border_val = inp_nodes[nx].threshold;
      nodes[nx].weight = inp_nodes[nx].weight;
  }
}
